CREATE OR REPLACE PROCEDURE DISHA_MART_ANALYTICS.REGULATORY_MART.LOAD_BUSINESS_MIS_MART_DISB_CANCELLED_YTD("START_DATE" DATE DEFAULT DATE_ADDMONTHSTODATE(-1, CURRENT_DATE()))
RETURNS VARCHAR(2550)
LANGUAGE SQL
EXECUTE AS CALLER
AS 'DECLARE 
	PROC_NAME VARCHAR(500):=''DISHA_MART_ANALYTICS.REGULATORY_MART.LOAD_BUSINESS_MIS_MART_DISB_CANCELLED_YTD'';
	TBL_NAME VARCHAR(500):=''DISHA_MART_ANALYTICS.REGULATORY_MART.DISB_CANCELLED_YTD'';
...
--Create DISB_CANCELLED_YTD table if not exists
CREATE TABLE IF NOT EXISTS DISHA_MART_ANALYTICS.REGULATORY_MART.DISB_CANCELLED_YTD (
	LOAN_NO	VARCHAR(255),
	LOAN_NO_NEW	VARCHAR(255),
	LOAN_ID	VARCHAR(255),
	...
	DISBURSAL_NO	NUMBER(38,0),
	DISBURSAL_AMOUNT	FLOAT,
	WIRR	FLOAT,
	FOIR	FLOAT,
	TENURE	FLOAT,
	LTV	FLOAT,
	DISBURSEMENT_CANCELLED_DATE	DATE,
	DISBURSAL_TYPE	VARCHAR(10),
	PRODUCT_DESC_L	VARCHAR(255),
	SCHEME_DESC_L	VARCHAR(13),
	HL_NHL	VARCHAR(255),
...
--Clean existing data for given month
	DELETE FROM DISHA_MART_ANALYTICS.REGULATORY_MART.DISB_CANCELLED_YTD 
...
--Create DISB_CANCELLED_YTD
CREATE OR REPLACE TEMPORARY TABLE DISHA_MART_ANALYTICS.REGULATORY_MART.Disbursement_cancelled AS
SELECT  
    TRIM(c.cod_Acct_no) as LAN_NEW,
    TRIM(b.cod_Acct_no_old) as loan_no,
    d.disb_no,
    DATE(d.dat_disb) as DISBURSAL_DATE,
    DATE(d.DAT_MARK_CANCEL) as Disbursement_cancelled_date,
    d.amt_disb As Cancel_Amount
FROM (SELECT * FROM REG_UAT_BACKUP.FLEX_LMS.LN_ACCT_DTLS WHERE ACTIVATION_FLAG=1) c
LEFT JOIN REG_UAT_BACKUP.FLEX_LMS.LN_X_OLD_NEW_ACCT_XREF b ON TRIM(c.cod_Acct_no) = TRIM(b.cod_Acct_no_new ) and b.ACTIVATION_FLAG=1
LEFT JOIN REG_UAT_BACKUP.FLEX_LMS.ln_x_disb_reg_mnt_hist d ON TRIM(c.cod_Acct_no) = TRIM(d.cod_Acct_no ) and d.ACTIVATION_FLAG=1
WHERE DATE(d.DAT_MARK_CANCEL) BETWEEN ''2025-04-01'' AND :MONTH_END_DATE;


CREATE OR REPLACE TEMPORARY TABLE DISHA_MART_ANALYTICS.REGULATORY_MART.DISB_CANCELLED_YTD_PREFINAL (
	LOAN_NO	VARCHAR(255),
	LOAN_NO_NEW	VARCHAR(255),
	LOAN_ID	VARCHAR(255),
	LOAN_APPLICATION_NAME	VARCHAR(511),
...
SELECT 
    ...
    TRIM(a.lan_new) as LOAN_NO_new,
    SUBSTR(TRIM(a.lan_new), 9, 6) as LOAN_ID,
    CASE 
        WHEN SUBSTR(TRIM(C.cod_agrmnt_no), 1, 2) = ''LA'' AND POSITION(''LA-'' IN TRIM(C.cod_agrmnt_no)) = 0
        THEN CONCAT(SUBSTR(TRIM(C.cod_agrmnt_no), 1, 2), ''-'', SUBSTR(TRIM(C.cod_agrmnt_no), 3)) 
        ELSE TRIM(C.cod_agrmnt_no)
    END as loan_application_name,
    MONTHNAME(a.DISBURSEMENT_CANCELLED_DATE) as Month,
    b.cod_cust_id as customer_id,
    b.nam_cust_full as CUSTOMER_NAME,
    CASE 
        WHEN TRIM(b.txt_cust_sex) = ''M'' THEN ''MALE'' 
        WHEN TRIM(b.txt_cust_sex) = ''F'' THEN ''FEMALE'' 
    END AS GENDER,
    g.nam_branch as Branch,
    g.NAM_CC_STATE as State,
    c.amt_face_value as LoanAmount,
    a.DISBURSAL_DATE,
    DATE(c.dat_first_disb) as FIRST_DISBURSAL_DATE,
    a.disb_no as DISBURSAL_NO,
    a.cancel_amount as DISBURSAL_AMOUNT,
    (br.irr * c.amt_face_value) / 100 as wirr,
    lac.Foir,
    c.CTR_TERM_MONTHS as Tenure,
    f.ltv_pct as LTV,
    a.DISBURSEMENT_CANCELLED_DATE,
    CASE WHEN a.disb_no = 1 THEN ''FIRST''
        WHEN a.disb_no > 1 THEN ''SUBSEQUENT'' 
    END as Disbursal_Type,
    d.nam_product as Product_Desc_L,
    CASE
        WHEN TRIM(e.txt_scheme) = ''G'' THEN ''General''
        WHEN TRIM(e.txt_scheme) = ''EL'' THEN ''Employee Loan''
        WHEN TRIM(e.txt_scheme) = ''S'' THEN ''STS''
        ELSE ''Not Found''
    END as Scheme_Desc_L,
    CASE 
        WHEN TRIM(d.cod_prod_category) = ''5'' THEN ''NHL''
        ELSE d.cod_prod_category
    END as HL_NHL,
	...
	lac.highest_income_profile
FROM DISHA_MART_ANALYTICS.REGULATORY_MART.disbursement_cancelled a
LEFT JOIN REG_UAT_BACKUP.FLEX_LMS.LN_ACCT_DTLS c ON TRIM(c.cod_acct_no) = TRIM(a.lan_new) and c.ACTIVATION_FLAG=1
LEFT JOIN REG_UAT_BACKUP.FLEX_LMS.ci_custmast b ON TRIM(b.cod_cust_id) = TRIM(c.cod_cust_id) and b.ACTIVATION_FLAG=1
LEFT JOIN REG_UAT_BACKUP.FLEX_LMS.LN_PROD_MAST d ON TRIM(d.cod_prod) = TRIM(c.cod_prod) and d.ACTIVATION_FLAG=1
LEFT JOIN REG_UAT_BACKUP.FLEX_LMS.LN_X_ADDLN_ATTRIBUTES e ON TRIM(e.COD_ACCT_NO) = TRIM(a.lan_new) and e.ACTIVATION_FLAG=1
LEFT JOIN REG_UAT_BACKUP.FLEX_LMS.LN_X_ACCT_LTV_DTLS f ON TRIM(f.cod_acct_no) = TRIM(a.lan_new) and f.ACTIVATION_FLAG=1
...
LEFT JOIN REG_UAT_BACKUP.FLEX_LMS.BA_CC_BRN_MAST g ON TRIM(g.cod_cc_brn) = TRIM(c.cod_cc_brn) and g.ACTIVATION_FLAG=1
LEFT JOIN REG_UAT_BACKUP.FLEX_LMS.CI_CUSTMAST_EXT j ON TRIM(j.cod_cust_id) = TRIM(c.cod_cust_id) and j.ACTIVATION_FLAG=1
...
-- Count total rows in prefinal table
    ...
    FROM DISHA_MART_ANALYTICS.REGULATORY_MART.DISB_CANCELLED_YTD_PREFINAL;
...
----IF TOTAL ROW COUNT 0 , Return Detailed Error
    ...
    AND TABLE_NAME = ''DISB_CANCELLED_YTD_PREFINAL''
...
INSERT_STATEMENT:= 
	CONCAT(''INSERT INTO DISHA_MART_ANALYTICS.REGULATORY_MART.DISB_CANCELLED_YTD('',
			INSERT_COLUMN_LIST,'')
			SELECT '',
			INSERT_VALUE_LIST,'','','''''''',MONTH_END_DATE,'''''''',
			'' FROM DISHA_MART_ANALYTICS.REGULATORY_MART.DISB_CANCELLED_YTD_PREFINAL S
			;'');
	
	EXECUTE IMMEDIATE :INSERT_STATEMENT;
	
-- Count rows affected
         ROWS_INSERTED := (SELECT COUNT(*) FROM DISHA_MART_ANALYTICS.REGULATORY_MART.DISB_CANCELLED_YTD WHERE REPORT_DATE=:MONTH_END_DATE);	
...
'',
            ''Status: SUCCESSFULLY LOADED DATA INTO DISB_CANCELLED_YTD''
        ) INTO :FINAL_OUTPUT;
